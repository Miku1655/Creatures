...
const ShopScreen = {
    ...
    openPack(contents) {
        ...
        for (let i = 0; i < contents.creatures; i++) {
            ...
            if (gameState.state.collection.length >= gameState.state.collectionLimit) {
                failed++;
                continue;
            }
        ...
        if (failed > 0) {
            UIManager.showNotification(`⚠️ Collection full! ${failed} creature(s) lost. Sell some creatures to make space!`, 'error');
        }
